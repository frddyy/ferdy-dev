...
// Helper function to robustly extract YouTube Video ID from any youtube URL format
...
export default async function ProjectDetail({ params }: Props) {
  ...
  // Check if link is YouTube and extract ID
  const links = Array.isArray(project.link) ? project.link : (project.link ? [project.link] : []);
  const youtubeLink = links.find(l => l.includes("youtube.com") || l.includes("youtu.be"));
  const isYouTube = !!youtubeLink;
  const youtubeId = youtubeLink ? getYouTubeId(youtubeLink) : null;

  return (
    // Added scroll-smooth so clicking the button smoothly glides down the page
    <main className="scroll-smooth min-h-screen pt-32 pb-32 px-6 max-w-[1200px] mx-auto relative overflow-hidden bg-white/40 dark:bg-black/40 backdrop-blur-xl rounded-[3rem] shadow-xl border border-white dark:border-white/10 mt-8 mb-8 transition-colors duration-500">
      ...
      <FadeIn delay={100}>
        <div className="flex flex-col">
                ...
                {links.length > 0 && (
                  <div className="pt-6 flex flex-col gap-3">
                    {links.map((linkUrl, idx) => {
                      const isThisYouTube = linkUrl.includes("youtube.com") || linkUrl.includes("youtu.be");
                      return (
                        <a 
                          key={idx}
                          href={isThisYouTube ? "#video-demo" : linkUrl} 
                          target={isThisYouTube ? "_self" : "_blank"} 
                          rel={isThisYouTube ? "" : "noopener noreferrer"} 
                        ...
                        >
                          ...
                          ) : (linkUrl.includes("vimeo.com") || linkUrl.includes("drive.google.com")) ? (
                            <>
          ...
          <div className="rounded-[2.5rem] overflow-hidden shadow-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#111] transition-colors mt-8">
            <ImageCarousel images={project.images} link={links.length > 0 ? links[0] : undefined} />
          </div>

          {/* Conditional YouTube Embed Section */}
          {isYouTube && youtubeId && (
            <div id="video-demo" className="mt-16 md:mt-24 pt-12 md:pt-16 border-t border-gray-200 dark:border-white/10 transition-colors">
              <div className="flex flex-col items-center justify-center mb-8 md:mb-12 text-center">
                <p className="text-xs font-bold text-gray-400 dark:text-gray-500 tracking-wider uppercase mb-2">Live Demonstration</p>
                <h3 className="text-3xl md:text-4xl font-bold text-[#1d1d1f] dark:text-white transition-colors">Watch {project.title} in action</h3>
              </div>
              
              <div className="relative w-full aspect-video rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl border border-gray-200 dark:border-white/10 bg-black">
                <iframe
